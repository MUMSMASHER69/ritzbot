...
class Cheat():
    ...
    def shuffle_deck(self):
        ...
        if self.count == 2:
            ...
            self.player1_dec, self.player2_dec, self.player3_dec = np.array_split(
                self.dec52, 3)
        else:
            self.player1_dec, self.player2_dec, self.player3_dec, self.player4_dec = np.array_split(
                self.dec52, 4)
        return
    ...
    async def join(self, message):
        if (message.author == self.player1 or message.author == self.player2 or message.author == self.player3 or message.author == self.player4):
            await message.channel.send("You have already joined the lobby")
            return
    ...
    async def gameplayLoop(self, message):
        if ((str(message.author) in self.strplayers) and self.gamestatus):
            if str(message.author) == str(self.playerturn):
                ...
                logger.info(str(self.playerturn))
                print(self.strplayers)
